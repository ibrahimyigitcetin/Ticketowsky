...
const SecurityGuardian = {
    ...
    audit(event, details, severity = 'info') {
        ...
        const auditPanel = document.getElementById('securityAuditList');
        if (auditPanel) this.renderAuditUI();
    },
...
function createTicket(ticketData) {
    ...
    if (!SecurityGuardian.verifyToken(ticketData.authenticity_token)) {
        SecurityGuardian.audit('CSRF Failure', 'Invalid or missing authenticity token during creation', 'critical');
...
function openDetailPanel(ticketId) {
    try {
        ...
        // Add Digital Twin Button dynamically if in detailPanel
        const detailPanel = document.getElementById('detailPanel');
        const actionsDiv = detailPanel.querySelector('.detail-actions');
        ...
        btn.onclick = (e) => {
            e.stopPropagation();
            window.dispatchEvent(new CustomEvent('open-sandbox', { detail: ticketId }));
        };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    SecurityGuardian.renderAuditUI();
    ...
    // Inject "Open Sandbox" button into detail panel if not present
    const detailActions = document.querySelector('.detail-actions');
    if (detailActions && !document.getElementById('openSandboxBtn')) {
        const btn = document.createElement('button');
        btn.id = 'openSandboxBtn';
        btn.className = 'btn-primary btn-sm';
        btn.style.background = 'linear-gradient(135deg, #ff4d00 0%, #ff0000 100%)';
        btn.innerHTML = '🧬 Otomatik İkiz (Sandbox)';
        ...
        setTimeout(() => {
            ...
            if (ticket) {
                updateTicket(targetId, {
                    ...
                    sandboxSession: null // Clear session upon successful deploy
                });
...
const SCENARIO_LIBRARY = {
    'database': {
        ...
        risks: [{
            ...
            alt: 'Tabloyu Online Rebuild Et',
            script: 'ALTER INDEX ALL ON [Users] REBUILD WITH (ONLINE = ON)'
        }]
    },
    'network': {
        name: 'Network Diagnostics & DNS Flush',
        steps: [
            'Probing DNS servers (8.8.8.8, 1.1.1.1)...',
            'Latency detected in internal VLAN 102...',
            'Flushing local DNS resolver cache...',
            'Renewing DHCP lease for interface [Killer-Wifi]...',
            'Tracing route to production-api.cloud.corp...',
            'Network convergence achieved.'
        ],
        risks: [{
            type: 'warning',
            msg: '<strong>⚠️ BAĞLANTI KESİNTİSİ:</strong> DNS Flush sırasında anlık gecikmeler yaşanabilir.',
            alt: 'Sadece Statik Kayıtları Yenile',
            script: 'ipconfig /registerdns && Resolve-DnsName -Name "Corp" -Server 10.0.0.1'
        }]
    },
    'ui': {
        name: 'Frontend Styles (CSS) & Layout Debug',
        steps: [
            'Analyzing DOM computed styles...',
            'Detected Z-Index collision in Header component...',
            'Recalculating layout boxes (Reflow)...',
            'Applying CSS patch: overflow-x: hidden !important;',
            'Verifying responsive breakpoints...',
            'UI Stability: Pixels Perfect.'
        ],
        risks: [{
            type: 'safe',
            msg: '<strong>✅ GÜVENLİ:</strong> Bu işlem sadece tarayıcı tarafında CSS günceller.',
            alt: 'Hard Code yerine CSS Variable kullan',
            script: 'Add-Styles --var-overrides "primary-color: #6366f1"'
        }]
    },
    'hardware': {
        name: 'Hardware Health & Thermal Check',
        steps: [
            'Querying SMART status from NVMe drive...',
            'Critical temp detected: 88°C on P-Cores...',
            'Injecting thermal throttling bypass...',
            'Testing fan speed response curve...',
            'Evaluating battery wear level: %85 health...',
            'Hardware Snapshot: Operational (Hot).'
        ],
        risks: [{
            type: 'critical',
            msg: '<strong>🔥 TERMİAL TEHLİKE:</strong> Fan devri artırımı geçici CPU yavaşlamasına neden olabilir.',
            alt: 'Cooling Pad simülasyonu başlat',
            script: 'Enable-ExternalCoolingMode -Target "P-Cores" -Limit 75C'
        }]
    },
    'security': {
        name: 'Security Audit & Patching',
        steps: [
            'Scanning for unauthorized registry modifications...',
            'Found suspicious entry in HKLM\\Software\\Microsoft\\Windows\\CurrentVersion\\Run',
            'Removing process: xmrig.exe...',
            'Updating Windows Defender pattern database...',
            'Hardening firewall INBOUND rules...',
            'Security Posture: Hardened.'
        ],
        risks: [{
            type: 'critical',
            msg: '<strong>🚫 ERİŞİM RİSKİ:</strong> Kayıt defteri silme işlemi sistem kararlılığını etkileyebilir.',
            alt: 'Sadece Karantinaya Al',
            script: 'Move-To-Quarantine -Source "RegistryKey" -Path "HKLM:\\Suspicious"'
        }]
    },
    'loadbalancer': {
        name: 'Load Balancer Reconfiguration',
        steps: [
            'Analyzing traffic weights on Node A/B/C...',
            'Detected session stickiness failure in Cluster 4...',
            'Reconfiguring HAProxy weighting algorithm...',
            'Purging edge-node cache...',
            'Rotating SSL certs for *.corp-api.net...',
            'Traffic balance: Optimized.'
        ],
        risks: [{
            type: 'warning',
            msg: '<strong>⚠️ TRAFİK KESİNTİSİ:</strong> Cache temizleme anlık gecikme yaratabilir.',
    ...
    'encryption': {
        ...
        steps: [
            'Initiating Vault session...',
            'Locating Master Key [MK-882]...',
            'Generating new AES-256 rotation cycle...',
            'Re-encrypting database field [ClientSecret]...',
            'Updating application env variables...',
            'Key Rotation: SUCCESS.'
        ],
        risks: [{
            type: 'critical',
            msg: '<strong>🔥 VERİ KAYBI RİSKİ:</strong> Eski keyler silinirse geçmiş datalar okunamaz!',
            alt: 'Geri Dönüş Noktası (Key Backup)',
            script: 'Backup-KeyVault -Snapshot "PreRotation" -VaultID 55'
        }]
    }
};
...
async function runSimulation(alternativeScript = null) {
    ...
    try {
        ...
        for (const line of lines) {
            ...
            if (!document.getElementById('sandboxModal').classList.contains('active')) break; // Stop loop if modal closed
...
function analyzeImpact(executedSteps, scenarioKey = null) {
    ...
    results.innerHTML = '';
    const scenario = SCENARIO_LIBRARY[scenarioKey] || SCENARIO_LIBRARY['database'];
    const currentRisks = risks.length > 0 ? risks : (scenario.risks || []);
    ...
    if (currentRisks.length > 0) {
        currentRisks.forEach(risk => {
            ...
            div.innerHTML = `
                <div class="impact-msg">${risk.msg}</div>
            `;
            results.appendChild(div);

            // Add Alternative Action button
            const altBtn = document.createElement('button');
            altBtn.className = 'btn-secondary btn-sm';
            altBtn.style.width = '100%';
            altBtn.style.marginTop = '10px';
            altBtn.innerHTML = `🛡️ Alternatif: ${risk.alt}`;
            altBtn.onclick = () => {
                showToast(`Alternatif Çözüm Devreye Alınıyor: ${risk.alt}`, 'info');
                runSimulation(risk.script);
            };
            alternativeActions.appendChild(altBtn);
        });

        // Add additional general safety suggestions
        const generalSugs = [
            { text: '🛠️ Tam Yedek Al', script: 'Backup-System -Full' },
            { text: '🔍 Derin Tarama Başlat', script: 'Start-DeepScan -Level 5' },
        ...
        results.innerHTML = '<div class="impact-safe">✅ DÜŞÜK RİSK: Bu işlem mevcut altyapı için güvenli görünüyor.</div>';
        alternativeActions.style.display = 'none';
    }
}
